...
(function (w) {
  ...
  async function fetchDaily(locId) {
    ...
    const url = `${base}/obs/daily/${locId}`; // ✅ bỏ provider
  ...
  function renderTrack() {
    ...
    steps.forEach((iso, idx) => {
      ...
      // ===== SOURCE COLOR: obs (blue) / fcst (yellow) / none (no color) =====
      const srcRaw =
        (series.sources && series.sources[idx]) ||
        (meta && meta.source) ||
        null;

      const src = typeof srcRaw === "string" ? srcRaw.toLowerCase() : null;

      // reset (đề phòng render lại)
      cell.classList.remove("fb-cell-obs", "fb-cell-fcst", "fb-cell-nodata");

      // gắn class theo source
      if (src === "obs") cell.classList.add("fb-cell-obs");
      else if (src === "fcst") cell.classList.add("fb-cell-fcst");
      else cell.classList.add("fb-cell-nodata"); // optional: để bạn dễ debug/định style
