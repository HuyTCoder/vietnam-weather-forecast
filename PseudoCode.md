...
FUNCTION HourlyScreen()
  ...
  IF weatherLoading AND weatherData is NULL THEN
    RENDER LoadingSpinner("Đang tải dự báo theo giờ"); RETURN
  END IF
  ...
  IF loadingSteps THEN
    RENDER LoadingSpinner("Đang tải dữ liệu timeseries"); RETURN
  END IF
  ...
  ON_CLICK "Tìm ngày/giờ":
    (selectedDate, selectedTime) = USER_PICK()
    IF missing -> ALERT("Chọn đủ ngày và giờ"); RETURN
...
FUNCTION OverviewScreen()
  ...
  ON_MOUNT:
    ...
    FINALLY loading = FALSE
  IF loading THEN RENDER LoadingSpinner; RETURN
  IF overviewData == NULL THEN RENDER "Không có dữ liệu"; RETURN
